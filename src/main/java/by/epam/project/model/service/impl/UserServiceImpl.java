...
import by.epam.project.model.PurchaseData;
...
public class UserServiceImpl implements UserService {
    ...
    public AjaxData createOrder(User user, List<Product> shoppingCart, String orderAddress,
        ...
        Order order = new Order(orderComment, orderAddress, new java.sql.Date(new Date().getTime()), totalPrice, Order.Status.NOT_CONFIRMED);
        try {
    ...
    @Override
    public AjaxData loadPurchaseData(Long id) throws ServiceException {
        AjaxData ajaxData = new AjaxData();

        try {
            List<PurchaseData> purchaseData = userDao.loadPurchaseData(id);

            java.sql.Date todaysDate = new java.sql.Date(new java.util.Date().getTime());
            List<PurchaseData> collection = new ArrayList<>();
            for(int i=0; i<28; i++){
                collection.add(new PurchaseData(subtractDays(todaysDate, i), 0));
            }

            collection.forEach(c->{
                purchaseData.forEach(p->{
                    if (p.getDateCreatedAt().toString().equals(c.getDateCreatedAt().toString())){
                        c.setCount(p.getCount());
                    }
                });
            });
            collection.sort(Comparator.comparing(PurchaseData::getDateCreatedAt));


            Map<String, List<PurchaseData>> mapResult = new LinkedHashMap<>();
            mapResult.put("firstWeek", collection.subList(0, 7));
            mapResult.put("secondWeek", collection.subList(7, 14));
            mapResult.put("thirdWeek", collection.subList(14, 21));
            mapResult.put("fourthWeek", collection.subList(21, 28));

            List<PurchaseData> fifthWeekCollection = new ArrayList<>();
            for(int i=0; i<7; i++){
                int count = (mapResult.get("firstWeek").get(i).getCount()
                        + mapResult.get("secondWeek").get(i).getCount()
                        + mapResult.get("thirdWeek").get(i).getCount()
                        + mapResult.get("fourthWeek").get(i).getCount())
                        / 4;
                fifthWeekCollection.add(new PurchaseData(addDays(todaysDate, i + 1), count));
            }
            mapResult.put("resultWeek", fifthWeekCollection);

            ajaxData.setJson(JsonUtil.toJson(mapResult));
    ...
    private java.sql.Date subtractDays(java.sql.Date date, int days) {
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        c.add(Calendar.DATE, -days);
        return new java.sql.Date(c.getTimeInMillis());
