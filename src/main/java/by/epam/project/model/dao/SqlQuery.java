...
public class SqlQuery {
    ...
    public static final String FIND_ALL_PRODUCTS_BY_CATEGORY_TO_CLIENT = "SELECT products.id as id, products.name AS name, " +
            "products.info AS info, product_statuses.name AS status, products.price AS price, products.count AS count, " +
     ...
     */
    public static final String UPDATE_PRODUCT_INFO = "UPDATE products SET name = ?, count = ?, info = ?, price = ? " +
            "WHERE id = ?";
     ...
     */
    public static final String UPDATE_PRODUCT_STATUS = "UPDATE products SET count = count-1, status_id = ? WHERE id = ?";
    public static final String UPDATE_PRODUCT_COUNT = "UPDATE products SET count = count-1 WHERE id = ?";
    ...
    public static final String LOAD_PURCHASE_DATA = "select o.time_created, op.product_id, count(*) as count from orders o" +
            " inner join orders_products op on o.id = op.order_id" +
            " where op.product_id=?" +
            " group by o.time_created, op.product_id";
    ...
    public static final String FIND_ALL_ORDER_PRODUCTS_BY_ID = "SELECT products.id as id, products.name as name, " +
            "info, price, count, image_url, product_statuses.name as status FROM products " +
            "JOIN product_statuses ON products.status_id = product_statuses.id " +
